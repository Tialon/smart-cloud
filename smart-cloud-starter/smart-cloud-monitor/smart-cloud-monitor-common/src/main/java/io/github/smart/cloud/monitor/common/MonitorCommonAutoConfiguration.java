...
public class MonitorCommonAutoConfiguration {
    ...
    @Bean
    @RefreshScope
    @ConfigurationProperties(prefix = "smart.monitor.proxy")
    public ProxyProperties proxyProperties() {
        return new ProxyProperties();
    }

    @Bean
    public WeworkRobotAgent weworkRobotAgent(final ProxyProperties proxyProperties) {
        return new WeworkRobotAgent(proxyProperties);
    }
